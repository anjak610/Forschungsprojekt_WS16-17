...
namespace Fusee.Tutorial.Core.Octree
{
    ...
    public class OctreeVSP<T>
    {
        public delegate void OnNewNodeAdded(OctreeNodeVSP<T> node);
        ...
        private OctreeNodeVSP<T> _root; // root is not really a bounding box, it just contains the eight cube voxels around the center point as children.
        ...
        public OctreeVSP(float3 position, float sideLength)
        {
        ...
        public void Add(float3 targetPosition, T data)
        {
            ...
            while (!found)
            {
                List<OctreeNodeVSP<T>> nodesAdded = new List<OctreeNodeVSP<T>>();
                found = _root.Add(ref targetPosition, ref data, out nodesAdded);

                if (nodesAdded != null)
                {
                    foreach (OctreeNodeVSP<T> nodeAdded in nodesAdded)
                    {
                ...
                if (!found)
                {
                    ...
                    for (var i = 0; i < 8; i++)
                    {
                        OctreeNodeVSP<T> childNode = _root.Children[i];
        ...
        {
            _root = new OctreeNodeVSP<T>(CenterPosition, MinSideLength * 2);
            ...
            for (var i = 0; i < 8; i++)
            {
                ...
                OctreeNodeVSP<T> node = new OctreeNodeVSP<T>(CenterPosition + position, MinSideLength);
                _root.AddChild(node);
        ...
        public static OctreeNodeVSP<T> Grow(OctreeNodeVSP<T> node)
        {
            OctreeNodeVSP<T> parentNode = CreateNodeWithSideLength(node.Position, node.SideLength * 2);
        ...
        public static OctreeNodeVSP<T> CreateNodeWithMinSideLength(float3 targetPosition)
        {
        ...
        public static OctreeNodeVSP<T> CreateNodeWithSideLength(float3 targetPosition, float sideLength)
        {
            ...
            OctreeNodeVSP<T> node = new OctreeNodeVSP<T>(newPos, sideLength);
