...
public class PointOctree<T> where T : class {
	...
	public PointOctree(float initialWorldSize, float3 initialWorldPos, float minNodeSize) {
	...
	public PointOctreeNode<T> GetOctreeNode(float3 target) {
        return rootNode.GetOctreeNode(ref target); ;
	}
    ...
    void Grow(float3 direction) {
		...
		for (int i = 0; i < 8; i++) {
			if (i == rootPos) {
				...
				children[i] = new PointOctreeNode<T>(rootNode.SideLength, minSize, newCenter + new float3(xDirection * half, yDirection * half, zDirection * half));
			}
		}
