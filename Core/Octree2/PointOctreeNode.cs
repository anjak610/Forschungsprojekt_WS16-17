...
public class PointOctreeNode<T> where T : class {
	...
	public PointOctreeNode(float baseLengthVal, float minSizeVal, float3 centerVal) {
    ...
    */
    ...
    /// <summary>
    /// Returns the octree node, in which the given point resides in.
    /// If none is found, it creates a new one.
    /// </summary>
    /// <param name="target">The position of the target point. Passing as ref to improve performance since it won't have to be copied.</param>
    /// <returns>The octree node in which the point resides in.</returns>
    public PointOctreeNode<T> GetOctreeNode(ref float3 objPos)
    {
        if (!Encapsulates(bounds, objPos))
        {
            return null;
        ...
        {
            PointOctreeNode<T> result = child.GetOctreeNode(ref objPos);
	...
	public void DrawAllBounds(float depth = 0) {
		...
		Bounds thisBounds = new Bounds(Center, new float3(SideLength, SideLength, SideLength));
	...
	*/
	...
	void SetValues(float baseLengthVal, float minSizeVal, float3 centerVal) {
		...
		childBounds[0] = new Bounds(Center + new float3(-quarter, quarter, -quarter), childActualSize);
		childBounds[1] = new Bounds(Center + new float3(quarter, quarter, -quarter), childActualSize);
		childBounds[2] = new Bounds(Center + new float3(-quarter, quarter, quarter), childActualSize);
		childBounds[3] = new Bounds(Center + new float3(quarter, quarter, quarter), childActualSize);
		childBounds[4] = new Bounds(Center + new float3(-quarter, -quarter, -quarter), childActualSize);
		childBounds[5] = new Bounds(Center + new float3(quarter, -quarter, -quarter), childActualSize);
		childBounds[6] = new Bounds(Center + new float3(-quarter, -quarter, quarter), childActualSize);
		childBounds[7] = new Bounds(Center + new float3(quarter, -quarter, quarter), childActualSize);
	}
	...
	void Split() {
		...
		children[0] = new PointOctreeNode<T>(newLength, minSize, Center + new float3(-quarter, quarter, -quarter));
		children[1] = new PointOctreeNode<T>(newLength, minSize, Center + new float3(quarter, quarter, -quarter));
		children[2] = new PointOctreeNode<T>(newLength, minSize, Center + new float3(-quarter, quarter, quarter));
		children[3] = new PointOctreeNode<T>(newLength, minSize, Center + new float3(quarter, quarter, quarter));
		children[4] = new PointOctreeNode<T>(newLength, minSize, Center + new float3(-quarter, -quarter, -quarter));
		children[5] = new PointOctreeNode<T>(newLength, minSize, Center + new float3(quarter, -quarter, -quarter));
		children[6] = new PointOctreeNode<T>(newLength, minSize, Center + new float3(-quarter, -quarter, quarter));
		children[7] = new PointOctreeNode<T>(newLength, minSize, Center + new float3(quarter, -quarter, quarter));
	}
	...
	static bool Encapsulates(Bounds outerBounds, float3 point) {
	...
	int BestFitChild(float3 objPos) {
    ...
    /*
	public static float DistanceToRay(Ray ray, float3 point) {
		return float3.Cross(ray.direction, point - ray.origin).magnitude;
	}
    */
}
